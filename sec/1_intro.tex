...
We use four post-hoc scoring methods (MSP, MaxLogit~\cite{maxlogit}, MaxEntropy~\cite{maxentropy}, and \textcolor{blue}{RbA (Rejected by All)~\cite{rba}}), and evaluate them on benchmarks from SMIYC~\cite{smiyc} and Fishyscapes~\cite{fishyscapes}. \textcolor{blue}{Temperature scaling is evaluated only for MSP and MaxEntropy, while RbA is reported without temperature scaling because the revised RbA score is based on the total known-class mass.}
