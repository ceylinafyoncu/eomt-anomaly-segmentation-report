\begin{table*}[!htbp]
\centering
\caption{Temperature scaling results across the three models. Only $T \in \{0.5, 1.0, 2.0\}$ shown; intermediate values $T \in \{0.75, 1.1, 1.5\}$ follow the same trend. AuPRC ($\uparrow$, \%) and FPR95 ($\downarrow$, \%) on five benchmarks. \textcolor{blue}{In the revised version, RbA is omitted from this temperature-scaling table because the final RbA score is not temperature-scaled.}}
\label{tab:temperature_combined}
\resizebox{\textwidth}{!}{
\begin{tabular}{llc|cc|cc|cc|cc|cc}
\toprule
 & & & \multicolumn{2}{c|}{RA-21} & \multicolumn{2}{c|}{RO-21} & \multicolumn{2}{c|}{FS L\&F} & \multicolumn{2}{c|}{FS Static} & \multicolumn{2}{c}{RoadAnom} \\
Model & Method & $T$ & AuPRC$\uparrow$ & FPR95$\downarrow$ & AuPRC$\uparrow$ & FPR95$\downarrow$ & AuPRC$\uparrow$ & FPR95$\downarrow$ & AuPRC$\uparrow$ & FPR95$\downarrow$ & AuPRC$\uparrow$ & FPR95$\downarrow$ \\
\midrule
ERFNet & MSP        & 0.5 & 27.05 & 62.75 & 2.42  & 63.24 & 1.28  & 66.73 & 6.60  & 43.48 & 12.19 & 82.04 \\
       &            & 1.0 & 29.09 & 62.57 & 2.71  & 65.27 & 1.75  & 50.62 & 7.47  & 41.84 & 12.42 & 82.59 \\
       &            & 2.0 & 30.60 & 65.05 & 3.00  & 72.74 & 2.69  & 47.92 & 9.54  & 41.29 & 12.64 & 84.58 \\
       & MaxEntropy & 0.5 & 28.58 & 62.74 & 2.64  & 63.73 & 1.59  & 52.82 & 7.04  & 43.20 & 12.33 & 82.26 \\
       &            & 1.0 & 30.96 & 62.67 & 3.04  & 65.95 & 2.58  & 50.19 & 8.84  & 41.55 & 12.67 & 82.76 \\
       &            & 2.0 & 30.27 & 67.09 & 2.97  & 75.98 & 3.51  & 47.10 & 11.72 & 41.75 & 12.79 & 85.60 \\
\midrule
EoMT-Cityscapes & MSP        & 0.5 & 74.21 & 33.73 & 90.10 & 0.55  & 25.40 & 14.91 & 54.33 & 34.94 & 75.51 & 19.37 \\
                &            & 1.0 & 74.36 & 33.73 & 90.11 & 0.55  & 25.42 & 14.90 & 54.35 & 34.94 & 75.69 & 19.31 \\
                &            & 2.0 & 74.55 & 33.73 & 90.11 & 0.55  & 25.44 & 14.90 & 54.36 & 34.93 & 75.87 & 19.28 \\
                & MaxEntropy & 0.5 & 77.43 & 33.68 & 90.09 & 0.59  & 29.55 & 14.83 & 53.21 & 34.91 & 78.43 & 19.03 \\
                &            & 1.0 & 77.88 & 33.60 & 90.01 & 0.68  & 29.56 & 14.72 & 52.51 & 34.84 & 78.14 & 18.79 \\
                &            & 2.0 & 77.91 & 33.53 & 89.97 & 0.72  & 29.01 & 14.65 & 51.46 & 34.77 & 77.43 & 18.63 \\
\midrule
EoMT-Fine-tuned & MSP        & 0.5 & 58.59 & 98.60 & 73.69 & 25.10 & 27.07 & 26.22 & 53.35 & 39.78 & 54.96 & 16.27 \\
                &            & 1.0 & 57.15 & 98.61 & 73.61 & 25.09 & 27.11 & 26.16 & 53.41 & 39.74 & 54.99 & 16.22 \\
                &            & 2.0 & 56.61 & 98.62 & 73.57 & 25.09 & 27.13 & 26.15 & 53.43 & 39.72 & 55.00 & 16.20 \\
                & MaxEntropy & 0.5 & 54.10 & 98.71 & 73.43 & 28.83 & 27.91 & 26.21 & 53.87 & 39.53 & 55.83 & 15.28 \\
                &            & 1.0 & 54.49 & 98.82 & 72.62 & 57.49 & 28.56 & 26.33 & 54.27 & 39.25 & 56.22 & 14.78 \\
                &            & 2.0 & 54.65 & 98.82 & 71.69 & 60.82 & 29.18 & 26.24 & 54.50 & 38.76 & 56.33 & 14.51 \\
\bottomrule
\end{tabular}}
\end{table*}
\begin{table}[!htbp]
\centering
\small
\caption{\textcolor{blue}{Revised RbA results for EoMT-COCO using the final Rejected by All known-class-mass score. AuPRC ($\uparrow$, \%) and FPR95 ($\downarrow$, \%) are reported on the five anomaly benchmarks.}}
\label{tab:rba_revised_coco}
\begin{tabular}{lcc}
\toprule
Dataset & AuPRC$\uparrow$ & FPR95$\downarrow$ \\
\midrule
RoadAnomaly21        & \textcolor{blue}{37.44} & \textcolor{blue}{62.24} \\
RoadObstacle21       & \textcolor{blue}{36.75} & \textcolor{blue}{99.99} \\
FS Lost\&Found       & \textcolor{blue}{0.28}  & \textcolor{blue}{96.62} \\
FS Static            & \textcolor{blue}{2.09}  & \textcolor{blue}{99.96} \\
RoadAnomaly          & \textcolor{blue}{20.64} & \textcolor{blue}{84.37} \\
\bottomrule
\end{tabular}
\end{table}
